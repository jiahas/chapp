...
import com.chapp.AllGattDescriptors
import com.chapp.AllGattServices
...
fun BluetoothGatt.printGattTable() {
    if (services.isEmpty()) {
        Log.i("GATT Table","No service and characteristic available, call discoverServices() first?")
        return
    }
    services.forEach { service ->
        val characteristicsTable = service.characteristics.joinToString(
            ...
            var description = "${AllGattCharacteristics.lookup(char.uuid)}: ${char.printProperties()}"
            if (char.descriptors.isNotEmpty()) {
                description += "\n" + char.descriptors.joinToString(
                    separator = "\n|------",
                    prefix = "|------"
                ) { descriptor ->
                    "${AllGattDescriptors.lookup(descriptor.uuid)}: ${descriptor.printProperties()}"
                }
            }
            description
        }
        Log.i("GATT Table","Service ${AllGattServices.lookup(service.uuid)}\nCharacteristics:\n$characteristicsTable")
    }
}
